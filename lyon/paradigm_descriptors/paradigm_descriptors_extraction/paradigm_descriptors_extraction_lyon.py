...
def open_logfile(sub_type, subject, task, session, run):
    ...
    input_file_path = os.path.join(my_path,
                                   '../../protocols/session_' + str(session) +
                                   '/loca_' + task + '_adapt/logfiles')

    # Path to subject logfiles
    if run == 1:
        run_path = os.path.join(input_file_path,
                                sub_type + subject + '_1-loca_' + task +
                                '.log')
    if run == 2:
        run_path = os.path.join(input_file_path,
                                sub_type + subject + '_2-loca_' + task +
                                '_bis.log')
...
def general_changes(subject, task, input_, response):
    # Extract TTL onset
    t0 = input_[
        (input_['Trial'] == 0) & (input_['Event Type'] == 'Pulse')].Time.values
    ...
    if task == 'audi':
        ...
        output_df = input_[input_['Event Type'] == 'Picture'][['Time',
                                                               'Duration',
                                                               'Code']]
    ...
    if task == 'audi':
        index_counter = 0
        index_list = output_df[output_df['trial_type']
                               == 'start_sound']['onset'].index

        for index in index_list:
            if index_counter == len(index_list) - 1:

                bfix_list = output_df[output_df['trial_type']
                                      == 'Bfix']['onset'].index
                ...
                output_df['duration'][index] = output_df['onset'] \
                                                   [index_list[
                        index_counter + 1]] - output_df['onset'] \
                                                   [index_list[index_counter]]
