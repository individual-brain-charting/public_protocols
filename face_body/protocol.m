function protocol(name, trigger, stim_set, num_runs, task_num, start_run)
% Executes IBC version of functional localizer experiment 
...
%     end
% end
num_runs = 8;  % Enter double the number of runs required %IBC change: longer runs
...
if nargin < 6
    start_run = -1;
    while ~ismember(start_run, 1:4)
        start_run = input('Start from run number (1-4) : ');
    end
end
...
for rr = start_run:num_runs/2   % IBC change: longer runs
