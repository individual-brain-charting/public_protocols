...
Order=3; % assigns randomization (IBC change: only using first randomization for all subjects)
...
logfile=[logsFolder sprintf('pract_NRPS_sub-%d.log',SubNumStr)];
OutputFile=[OutputsFolder sprintf('pract_sub-%d_task-%s_run-0%d.txt',SubNumStr,Task,run_number)];
...
% Show Instructions
line1 = 'Appuyez sur U pour << accepter fortement >>';
line2 = '\n\n Appuyez sur I pour << accepter faiblement >>';
line3 = '\n\n Appuyez sur O pour << rejeter faiblement >>';
line4 = '\n\n Appuyez sur P pour << rejeter fortement >>';
line5 = '\n\n Appuyez sur Espace pour commencer.';
DrawFormattedText(w, [line1 line2 line3 line4 line5],...
    'center', ycenter * 0.6, black);
Screen(w,'Flip');
...
for trial=1:n_trials
    ...
    Screen('DrawText',w,sprintf('+%d',Prospect{run_number,Order}(50+trial,2)),xcenter-150,ycenter-20,green); %gain: for row t (corresponds to trial #) in column 1 (run #)
    Screen('DrawText',w,sprintf('-%d',Prospect{run_number,Order}(50+trial,3)),xcenter+40,ycenter-20,red); %loss: for row t (corresponds to trial #) in column 1 (run #)
    ...
    fprintf(fid1,'%d\t%d\t%0.3f\t%d\t%d\t%0.3f\t%s\n',trial,stimons{run_number,Order}(trial),rt(trial,1),
        Prospect{run_number,Order}(50+trial,2),-1*Prospect{run_number,Order}(50+trial,3),rt(trial,2),resp{trial});
    ...
    Prospect{run_number,Order}(50+trial,2),-1*Prospect{run_number,Order}(50+trial,3),gain_side,rt(trial,2),
