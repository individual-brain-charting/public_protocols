...
menu_options = [setting_mm["instruct"], setting_mm["demo_sess"],
                ...
                setting_mm["exit"]]
note = "Attention: Attendez quelques instants après avoir sélectionné " + \
       "votre option, SVP."
display_note = stimuli.TextLine(note.decode('utf-8'), text_size=24,
                                text_colour=(0, 0, 150), position=(0, -300))
...
while True:
    ...
    if selected_option < (len(menu_options) - 1):
        exp = design.Experiment('language_processing',
                                ...
                                background_colour=(127, 127, 127))
        control.initialize(exp)
        menu_options = ["Instructions", "Session de démonstration",
                        "Session d'entraînement", "Expérience principale",
                        "Sortie"]
        menu = io.TextMenu("Protocole de Langage", menu_options, width=1000,
                           text_size=28, gap=10,
                           background_colour=(127, 127, 127),
                           background_stimulus=display_note)
        selected_option = menu.get(3)
