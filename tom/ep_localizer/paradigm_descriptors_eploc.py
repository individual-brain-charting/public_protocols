"""
Script for paradigm descriptors' extraction of the ep_loc task
    ...
    for run in np.arange(1, 3):
        ...
        for r, row in enumerate(log_mat_flatten):
            ...
            if log_mat_flatten[r - 4] == "# name: design":
                ...
                name = ["emotional_pain" if j == "1" else
                        "physical_pain" for j in name_code]
            # Extract onsets
            ...
            elif log_mat_flatten[r - 2] == "# name: responseDur":
                response_duration = int(row)

        # Create array for duration
        duration = [story_duration + response_duration] * len(onset)
