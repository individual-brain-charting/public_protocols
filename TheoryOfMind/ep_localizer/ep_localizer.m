function ep_localizer(subjID, run)
...
orig_dir			= pwd;
stimdir			  = fullfile(rootdir, 'stimuli_french');
...
try
  ...
  Screen(w, 'TextStyle', 1);
    ...
    Screen(w, 'DrawText', instr_1, x0-350, y0-110, [255]);
    Screen(w, 'DrawText', instr_2, x0-350, y0-80,[255]);
    Screen(w, 'DrawText', instr_3, x0-350, y0-50,[255]);
    Screen(w, 'DrawText', instr_4, x0-350, y0+30, [255]);
    ...
    Screen(w, 'DrawText', instr_5, x0-350, y0+70, [255]);
    Screen(w, 'Flip');			% Instructional screen is presented.
    while 1
      [keyIsDown, secs, keyCode] = KbCheck;
    ...
    Screen(w, 'DrawText', instr_6, x0-225, y0+70, [255]);
    Screen(w, 'Flip');			% Instructional screen is presented.
    while 1
      [keyIsDown, secs, keyCode] = KbCheck;
    ...
    Screen(w, 'DrawText', instr_7, x0-100, y0+70, [255]);
    Screen(w, 'Flip');			% Instructional screen is presented.
    while 1
      [keyIsDown, secs, keyCode] = KbCheck;
    ...
    Screen(w, 'DrawText', instr_8, x0+25, y0+70, [255]);
	  Screen(w, 'Flip');		 % Instructional screen is presented.
...
try
	for trial = 1:trialsPerRun
      ...
      assert(keyCode(key_escape) == 0);     % raise exception if ESC key was pressed
        ...
        Screen(w,'TextSize',22);
        Screen(w,'DrawText',line_1,x0-350,y0-170,255);
        Screen(w,'DrawText',line_2,x0-350,y0-140,255);
        Screen(w,'DrawText',line_3,x0-350,y0-110,255);
        Screen(w,'DrawText',line_4,x0-350,y0-80,255);
        Screen(w,'DrawText',line_5,x0-350,y0-50,255);
        ...
        Screen(w,'DrawText',line_4,x0-350,y0-80,255);
        Screen(w,'DrawText',line_5,x0-350,y0-50,255);
        Screen(w,'DrawText',question,x0-350, y0+30,255);
        Screen(w,'DrawText',answers,x0-350,y0+70,255);
