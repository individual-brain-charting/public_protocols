function tom_localizer_ts(subjID, run)
...
orig_dir			= pwd;
textdir				= fullfile(rootdir, 'training_sess_french');
...
try
  ...
  Screen('Preference','TextEncodingLocale','UTF-8');  
	...
	Screen(w, 'TextSize', 60);
  Screen(w, 'TextStyle', 1);
	task		= sprintf('Vrai ou Faux?');
	Screen(w, 'DrawText', task, x0-200, y0, [255]);
  ...
  Screen(w, 'DrawText', instr_1, x0-600, y0, [255]);
  Screen(w, 'Flip');
  while 1
  ...
  Screen(w, 'DrawText', instr_2, x0-650, y0, [255]);
  Screen(w, 'Flip');
...
try
	for trial = 1:trialsPerRun
		...
		storyname		= sprintf('%d%s_ts_story.txt',numbeT,condPrefs{trialT});
		questname		= sprintf('%d%s_ts_question.txt',numbeT,condPrefs{trialT});
      ...
      Screen(w, 'TextStyle', 1);
			Screen(w, 'DrawText',tline,x0-900,y0-160+lCounter*80,[255]);
			lCounter	= lCounter + 1;
		end
		fclose(textfid);
